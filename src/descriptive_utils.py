...
class GradingOutput(BaseModel):
    items: List[GradedItem]  # any length (0..N)


def to_T_keys(parsed: GradingOutput, start_index: int = 1) -> Dict[str, Any]:
    """Flatten without padding: only T1..Tn for n=len(items)."""
    out: Dict[str, Any] = {}
    for i, item in enumerate(parsed.items, start=start_index):
        out[f"extract_answer_T{i}"] = item.extract_answer
        out[f"score_T{i}"] = item.score
    return out


def get_descriptive_result_gpt_4o_mini(client, prompt, length, i, max_retries=10):
    curr_retries = 0
    max_tokens = 256
    while curr_retries < max_retries:
        try:
            ...
            print(f"Error at Query ID: {i} | {e}")
            try:
                print(f"Response: {response}")
            except:
                pass
            # increase the max_tokens if the response is too long
            if any(text in str(e) for text in ['Unterminated string starting at', 'Expecting value']):
                if max_tokens >= 1024:
...
# def get_descriptive_result_gpt_4o_mini(client, prompt, length, query_idx, max_retries=5):
#     curr_retries = 0
#     max_tokens = 256
#     while curr_retries < max_retries:
#         try:
...
#             )
#             breakpoint()
#             content = to_T_keys(response.output_parsed)
#             verify_grading_output(content, length)
#             break
#         except Exception as e:
#             print(f"Error at Query ID: {query_idx} | {e}")
#             # increase the max_tokens if the response is truncated (various JSON parsing errors)
#             if any(phrase in str(e) for phrase in ['Unterminated string starting at', 'EOF while parsing', 'json_invalid', 'GradingOutput']):
#                 try:
...
def safe_load_json(text, default=None):
    # Extract patterns and build result in DEFAULT_OUTPUT order
    result = {}
    is_missing = False
    
    for i in range(1, 6):  # T1 through T5
        # Extract answer
        extract_match = re.search(rf'"extract_answer_T{i}":\s*"([^"]*)"', text)
        if extract_match:
            value = extract_match.group(1).strip().strip('"').strip()
            if value == "":
                is_missing = True
            result[f"extract_answer_T{i}"] = value 
        else:
            is_missing = True
            result[f"extract_answer_T{i}"] = ""

        # Extract score
        score_match = re.search(rf'"score_T{i}":\s*([^,\]\}}]+)', text)
        if score_match:
            try:
                score_value = int(score_match.group(1).strip())
            except ValueError:
                score_value = 0
                is_missing = True
        else:
            score_value = 0
            is_missing = True
        # Treat out-of-range scores as missing
        if score_value not in (0, 1):
            is_missing = True
        result[f"score_T{i}"] = score_value
    
    # If any fields are missing, return the provided default (e.g., sentinel) to trigger regeneration
    if is_missing and default is not None:
        return default
...
def get_descriptive_result_gemma(client, prompt, length, max_retries=50):
    ...
    while curr_retries < max_retries:
        try:
            ...
            # Retry generation if parsing fails (up to 5 parse retries, independent of max_retries)
            sentinel = object()
            content = safe_load_json(response.text, default=sentinel)
            if content is sentinel:
                print(f"Failed to parse response text or missing required fields: {response.text}, Retrying...")
                parse_failures += 1
